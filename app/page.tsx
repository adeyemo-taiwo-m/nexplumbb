...
import { Search, MapPin, ShieldCheck, Lock, Star, ArrowRight, Zap, CheckCircle, Users, Clock } from 'lucide-react'
...
export default function Homepage() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    router.push(`/search?q=${encodeURIComponent(searchQuery.trim())}&location=${encodeURIComponent(customLocation)}`)
  }
  ...
  const trustMetrics = [
    { icon: ShieldCheck, value: '1,200+', label: 'Verified Artisans', color: 'teal' },
    { icon: Lock, value: 'Escrow', label: 'Protected Payments', color: 'orange' },
  ...
  const seoSections = [
    { id: 'plumbers-in-lagos', title: 'Plumbers in Lagos', desc: 'Access highly rated plumbing professionals for leak repairs, drain unblocking, faucet installations, and pipe layouts. Secure escrow payments ensure full service satisfaction.', trade: 'Plumbing', label: 'plumbers' },
    { id: 'electricians-in-lagos', title: 'Electricians in Lagos', desc: 'Expert electricians for domestic re-wiring, generator changeover switches, socket replacements, and troubleshooting power outages. Fast response and fully verified profiles.', trade: 'Electrical', label: 'electricians' },
    { id: 'carpenters-in-lagos', title: 'Carpenters in Lagos', desc: 'Custom woodworking, cabinet makers, door installations, and roof repairs. Connecting you to verified local carpenters with guaranteed expertise across Lagos.', trade: 'Carpentry', label: 'carpenters' },
    { id: 'painters-in-lagos', title: 'Painters in Lagos', desc: 'Professional screeding and painting services for residential and commercial layouts. Transform your spaces with vetted local artisans offering transparent pricing.', trade: 'Painting', label: 'painters' },
    { id: 'tilers-in-lagos', title: 'Tilers in Lagos', desc: 'Porcelain, ceramic, marble, and mosaic tile professionals. Floor tiling, wall tiling, and bathroom renovations by trusted, identity-verified artisans.', trade: 'Tiling', label: 'tilers' }
  ]

  const socialProofReviews = [
    { name: 'Chisom A.', area: 'Surulere', stars: 5, text: 'Nexplumb completely saved my day! A kitchen pipe burst at 9:00 AM. I booked Emeka, tracked him, and by 11:30 AM the pipe was replaced. The escrow system felt super secure!' },
    { name: 'Kunle O.', area: 'Yaba', stars: 4, text: 'Very smooth experience. Tunde was professional, did neat wiring, and cleared up afterwards. Love that I can pay by bank transfer directly on the app.' },
    { name: 'Nkechi E.', area: 'Lekki', stars: 5, text: 'Having a verified artisan come into my home makes me feel safe. Knowing their NIN has been checked by Nexplumb is a game changer for safety in Lagos.' }
  ]
      ...
      <main className="flex-grow flex flex-col">
            ...
            <div className="hidden desktop:flex desktop:col-span-5 flex-col items-center justify-center relative">
              <div className="animate-fade-in-up delay-300 w-full">
                <div className="glass-card rounded-modal p-8 text-center relative overflow-hidden">
                  ...
                  <h3 className="text-white text-[20px] font-display font-bold">Trust-First Platform</h3>
                  <p className="text-white/65 font-body text-[14px] mt-2 max-w-[280px] mx-auto leading-relaxed">
                    Every artisan undergoes NIMC NIN checks, BVN verification, and guarantor audits.
                  </p>
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-6">
                    {[
                      ...
                      <div key={stat.label} className="bg-white/5 rounded-card py-3 px-2 border border-white/8">
                        <p className="font-mono text-[14px] font-bold text-white">{stat.value}</p>
                        <p className="font-mono text-[9px] text-white/50 uppercase tracking-wider mt-0.5">{stat.label}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <section className="w-full bg-gradient-to-r from-[#0A1A2B] to-[#0D2137] border-t border-white/5 py-8">
          <div className="max-w-[1200px] mx-auto px-6 grid grid-cols-2 tablet:grid-cols-4 gap-6">
            {trustMetrics.map((metric, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center gap-3 text-white animate-fade-in-up" style={{ animationDelay: `${idx * 100}ms` }}>
                  <div className={`p-2.5 rounded-card bg-${metric.color}/10 text-${metric.color} border border-${metric.color}/15`}>
                    <Icon size={20} />
                  </div>
                  <div>
                    <p className="text-[18px] font-display font-bold leading-none">{metric.value}</p>
                    <p className="text-[10px] font-mono text-white/55 mt-1 uppercase tracking-wider">{metric.label}</p>
                  </div>
                </div>
              )
            })}
        ...
        <section id="how-it-works" className="w-full bg-white py-24 border-b border-border">
          <div className="max-w-[1200px] mx-auto px-6 tablet:px-10 text-center">
            <span className="inline-block font-mono text-[11px] font-bold text-teal uppercase tracking-widest mb-3">Simple Process</span>
            ...
            <div className="grid grid-cols-1 tablet:grid-cols-3 gap-6 text-left">
              {steps.map((step, idx) => {
                ...
                return (
                  <div key={step.number} className="card-hover-lift flex flex-col items-start p-7 rounded-card border border-border bg-lgray/30 group">
            ...
            <div className="mt-16">
              <Link href="/search">
                <Button variant="primary" size="lg" className="px-10 py-4 shadow-lg hover:shadow-xl transition-shadow">
                  Find an artisan now
                </Button>
              </Link>
            </div>
        ...
        <section className="w-full bg-lgray/30 py-24 border-b border-border">
          <div className="max-w-[1200px] mx-auto px-6 tablet:px-10">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-12 gap-4">
              <div>
                <span className="inline-block font-mono text-[11px] font-bold text-teal uppercase tracking-widest mb-2">Top Rated</span>
              ...
              <Link href="/search" className="font-mono text-[13px] text-nxblue font-bold hover:underline flex items-center gap-1.5 select-none group">
                See all artisans <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
            ...
            {loadingArtisans ? (
              ...
              <div className="bg-white border border-border rounded-card p-12 text-center">
                <p className="text-slate font-body">No artisans found yet. Be the first to sign up!</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 desktop:grid-cols-4 gap-6">
                {featuredArtisans.map((artisan) => (
                  <ArtisanCard key={artisan.id} variant="vertical" {...artisan} />
                ))}
              </div>
            )}
          </div>
        ...
        {seoSections.map((sec, index) => {
          const matchingArtisans = artisans.filter((a) => a.trade === sec.trade).slice(0, 2)
          return (
            <section key={sec.id} id={sec.id} className={`w-full py-20 border-b border-border ${index % 2 === 0 ? 'bg-white' : 'bg-lgray/20'}`}>
              <div className="max-w-[1200px] mx-auto px-6 tablet:px-10 grid grid-cols-1 desktop:grid-cols-12 gap-10 items-center">
                <div className="desktop:col-span-5">
                  <span className="inline-block font-mono text-[11px] font-bold text-teal uppercase tracking-widest mb-2">Trade Category</span>
                  <h2 className="text-h2 text-navy mb-4">{sec.title}</h2>
                  <p className="font-body text-[15px] text-body leading-relaxed max-w-[480px]">{sec.desc}</p>
                  <div className="mt-5 flex flex-wrap gap-2 select-none">
                    <span className="font-mono text-[10px] font-bold bg-teal/8 text-teal border border-teal/15 px-3 py-1.5 rounded-full">Escrow Guarded</span>
                    <span className="font-mono text-[10px] font-bold bg-nxblue/8 text-nxblue border border-nxblue/15 px-3 py-1.5 rounded-full">NIN Verified</span>
                  </div>
                  <button type="button" onClick={() => handleCategoryClick(sec.trade)} className="mt-6 font-mono text-[13px] font-bold text-orange hover:underline flex items-center gap-1 group focus-visible:outline focus-visible:outline-2 focus-visible:outline-teal">
                    Find more {sec.label} near you <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
                  </button>
                </div>
        ...
        <section className="w-full bg-white py-24">
          <div className="max-w-[1200px] mx-auto px-6 tablet:px-10 text-center">
            <span className="inline-block font-mono text-[11px] font-bold text-teal uppercase tracking-widest mb-3">Testimonials</span>
            <h2 className="text-h2 text-navy mb-14">What Lagos residents say</h2>
            
            <div className="grid grid-cols-1 tablet:grid-cols-3 gap-6 text-left">
              {socialProofReviews.map((rev, idx) => (
                <div key={idx} className="card-hover-lift bg-lgray/40 rounded-card p-6 border border-border flex flex-col justify-between">
                  <div>
                    ...
                    <p className="font-body text-[14px] text-body italic leading-relaxed">&ldquo;{rev.text}&rdquo;</p>
                  </div>
                  <div className="mt-6 flex items-center gap-3 border-t border-border pt-4">
                    ...
                    <div>
                      <h4 className="font-display font-bold text-[13px] text-navy leading-none">{rev.name}</h4>
                      <p className="font-mono text-[10px] text-slate mt-1">{rev.area}, Lagos</p>
                    </div>
                  </div>
                </div>
              ))}
